...
{
public:
	A(){printf("Construct A() id=%d %p\n",m_id, this);};
	A(int id){m_id=id;printf("Construct A(int) id=%d %p\n",m_id, this);};
	virtual ~A(){printf("Destruct A id=%d %p\n",m_id, this);};
	int m_id;
	bool operator==(const A& a){return (this->m_id==a.m_id);};
};
class Aderived: public A
{
public:
	Aderived(){printf("Construct Aderived() id=%d %p\n",m_id, this);};
	~Aderived(){printf("destructor of Aderived~\n");};
};
