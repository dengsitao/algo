...
public:
    vector<vector<string> > findLadders(string beginWord, string endWord, vector<string>& wordList) {
        ...
        createGraph(beginWord, wordList, endWord, 1);
...
private:
    ...
    void updateGraph(string k, string con) {
        map<string, vector<string> >::iterator citr1 = count.find(k);
        map<string, vector<string> >::iterator citr = count.find(con);
        int baseDepth=citr1->second.size();
        if (citr == count.end()) {
            count.insert(pair<string, vector<string> >(con, curAns));
            printf("adding %s to count %d\n", con.c_str(), baseDepth+1);
        } else if (baseDepth+1 < citr->second.size()) {
            count.erase(con);
            count.insert(pair<string, vector<string> >(con, curAns));
            printf("replacing %s's con %s size=%d to count\n", k.c_str(), con.c_str(), baseDepth+1);
        }
        else {
            printf("%s has a shorter way %lu, depth=%d\n", con.c_str(), citr->second.size(), baseDepth+1);
        }
    ...
    void createGraph(string k, vector<string> wordList, string endWord, int d) {
        if (k == endWord) return;
        ...
        while (it != cons.end()) {
            createGraph(*it, wordList, endWord, d+1);
            it++;
        }
