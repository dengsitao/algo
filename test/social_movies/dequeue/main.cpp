...
typename TypeContainer::iterator findInContainer(TypeData data, TypeContainer & container, CompFunc isSame)
{
    typename TypeContainer::iterator itr = container.begin();

    while (itr != container.end() && !(*isSame)(data, (*itr))) {
        itr++;
    }
...
SocialMovies::~SocialMovies()
{
    deleteAllFromContainer(m_customerList, deletePointer<CustomerInfo>);
    deleteAllFromContainer(m_movieList, deletePointer<MovieInfo>);
}
...
{
    std::deque<MovieInfo*>::iterator itr = findInContainer(movieId, m_movieList, isIdSame<MovieInfo*>);
...
{
    std::deque<CustomerInfo*>::iterator itr = findInContainer(customerId, m_customerList, isIdSame<CustomerInfo*>);
...
{
    std::deque<MovieInfo*>::iterator movie_itr = findInContainer(movieId, m_movieList, isIdSame<MovieInfo*>);
    std::deque<CustomerInfo*>::iterator customer_itr = findInContainer(customerId, m_customerList, isIdSame<CustomerInfo*>);
...
{
    std::deque<CustomerInfo*>::iterator itr1 = findInContainer(customerId1, m_customerList, isIdSame<CustomerInfo*>);
    std::deque<CustomerInfo*>::iterator itr2 = findInContainer(customerId2, m_customerList, isIdSame<CustomerInfo*>);
...
int compareTwoCustomer(CustomerInfo & ct1, CustomerInfo & ct2)
{
    ...
    for (std::deque<MovieInfo*>::iterator itr1 = ct1.m_likedMovies.begin(); itr1 != ct1.m_likedMovies.end(); itr1++) {
        for (std::deque<MovieInfo*>::iterator itr2 = ct2.m_likedMovies.begin(); itr2 != ct2.m_likedMovies.end(); itr2++) {
...
ResultCode SocialMovies::getSimilarCustomerInNetwork(int customerId, int maxDistance, int& similarCustomerId)
{
    ...
    std::deque<CustomerInfo*>::iterator itr = customer_itr;
...
{
    for (std::deque<MovieInfo*>::iterator itr = cus.m_likedMovies.begin(); itr != cus.m_likedMovies.end(); itr++) {
        if (movieId == (*itr)->m_Id) {
            return true;
        }
    }
...
ResultCode SocialMovies::getMostLikedMovies(int maximumResults, std::vector<int>& mostLikedMovieIds)
{
    ...
    for (std::deque<MovieInfo*>::iterator mitr = m_movieList.begin(); mitr != m_movieList.end(); mitr++) {
        int likedCount = 0;

        for (std::deque<CustomerInfo*>::iterator citr = m_customerList.begin(); citr != m_customerList.end(); citr++) {
